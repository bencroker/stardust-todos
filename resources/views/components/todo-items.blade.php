<div id="main" class="min-w-screen min-h-screen bg-gray-800 flex items-center justify-center px-5 py-5">
    <div class="w-full mx-auto rounded-lg border border-gray-700 p-8 lg:py-12 lg:px-14 text-gray-300" style="max-width: 800px">
        ...
        <div class="mb-10">
                    ...
                    @foreach($todoItems as $item)
                        <li class="px-2 py-2 rounded transition-all flex text-md">
                            <div class="flex-none w-10 leading-none">
                                <input type="checkbox" data-on:change="{{ datastar()->action(['TodosController', 'toggleStatus'], ['id' => $item['id'], 'currentStatus' => $item['status']]) }}"
            ...
            @endif
            <input data-on:change="{{ datastar()->action(['TodosController', 'create']) }}" data-bind:title type="text" class="text-md w-full bg-transparent text-gray-300 leading-none focus:outline-none mb-2" placeholder="New todo..."/>
        </div>
